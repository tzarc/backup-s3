...
if [[ ! -z $DB_NAME ]]; then
	...
	aws s3 cp s3://$S3_BUCKET/postgres/$BACKUPNAME.sql.gz /tmp/dump.sql.gz
...
if [[ ! -z $DATA_PATH ]]; then
	...
	aws s3 cp s3://$S3_BUCKET/data/$BACKUPNAME.tar /tmp/data.tar
	...
	tar xf /tmp/data.tar --absolute-names --no-overwrite-dir --no-same-owner
